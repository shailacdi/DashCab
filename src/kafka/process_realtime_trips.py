...
number of trips per timeblock, joined historical stats to get historical mean and std_dev
...
"assign_date,borough_code,time_block,month,day,actual_trips,borough_name,mean,std_dev int"
...
def process_stream(rdd, real_trip_table,real_trip_stats_table,keyspace):
    ...
    dataRaw = rdd.map(lambda x: x[1]) \
                .map(lambda row: util.process_real_trip_record(row,borough_info.value)) \
    ...
    dataRaw.toDF(schema=["assign_date","hack_license","medallion_id","time_block","month","day","borough_code","borough_name","long","lat"],sampleRatio=0.2).write.format("org.apache.spark.sql.cassandra").mode("append").options(table=real_trip_table, keyspace=keyspace).save()
    ...
    dataRaw_df.join(stats_df,["borough_code","time_block"]) \
        .write.format("org.apache.spark.sql.cassandra").mode("append").options(table=real_trip_stats_table, keyspace=keyspace).save()
...
if __name__ == '__main__':
    ...
    query_str = "select time_block, borough_code, mean, std_dev from {0} where day='{1}' and month='{2}' allow filtering".format(trip_stats_table,day,month)
    ...
    borough_info = sc.broadcast(util.get_borough_data_dict(borough_file))
